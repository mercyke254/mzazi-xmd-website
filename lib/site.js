...
//
// The public addresses come from the environment and are read through lib/env.js,
// which tolerates the ways an environment variable gets written wrong (quotes, a
// missing protocol, a trailing slash) instead of failing on them. They fall back
// to literals rather than to the raw value, because they are passed to `new URL()`
// for metadata and an unparseable value there takes down every page.
//
// Everything else this site needs — the database and the session secret — lives in
// the server-only environment and is read by lib/db.js and lib/session.js. None of
// it appears here on purpose: this file is imported by client components too, and
// a secret that reaches a client bundle has leaked.
// ─────────────────────────────────────────────────────────────────────────────

import { normaliseUrl } from './env';

const SITE_FALLBACK = 'https://mzazi.shop';
const ACCOUNT_FALLBACK = 'https://www.mzazi.shop';

const siteUrl = normaliseUrl(process.env.NEXT_PUBLIC_SITE_URL, { fallback: SITE_FALLBACK });
const accountBase = normaliseUrl(process.env.NEXT_PUBLIC_MZAZI_SITE, { fallback: ACCOUNT_FALLBACK });
