...
//
// JWT_SECRET must therefore be the SAME value on both deployments. It is the only
// shared secret this site needs.
// ─────────────────────────────────────────────────────────────────────────────
...
function secret() {
  ...
  if (!value) {
    throw new ConfigError(
      'JWT_SECRET is not set on this deployment. Use the same value as the platform site so a session works on both.'
    );
  }
  return value;
}
...
export async function sessionClaims() {
  try {
    ...
    // A missing secret lands here too, which is correct: without it no session
    // can be trusted, so the caller gets null rather than an authenticated user.
    return null;
  }
}
